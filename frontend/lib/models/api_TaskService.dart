...
class ApiTaskService {
  ...
  Future<List<TaskModel>> getAllTasks() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return Future.error(
            'Không thể tải danh sách task: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<bool> moveTask(
      String taskId, String newListId, String newBoardId) async {
  ...
  Future<List<dynamic>> getAllTasksByListId(String idList) async {
    try {
      final response = await http.get(
          Uri.parse('$baseUrl/api/task/l/$idList')); // Sửa URL đúng với route
      if (response.statusCode == 200) {
        final Map<String, dynamic> json = jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> getTaskById(String id) async {
    try {
      final response = await http
          .get(Uri.parse('$baseUrl/api/task/$id')); // Sửa URL đúng với route
  ...
  Future<List<dynamic>> getBoardMembers(String boardId) async {
    try {
      final response =
          await http.get(Uri.parse('$baseUrl/api/board/$boardId/members'));
      ...
      if (response.statusCode == 200) {
        ...
        return Future.error(
            'Không thể tải danh sách thành viên: ${response.statusCode}');
      }
    } catch (e) {
      return Future.error('Lỗi: $e');
    }
  ...
  Future<bool> deleteTask(String id) async {
    try {
      final response = await http.delete(
          Uri.parse('$baseUrl/api/task/delete/$id')); // Sửa URL đúng với route
      if (response.statusCode == 200) {
        return true;
      } else {
        return false;
